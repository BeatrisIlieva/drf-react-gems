            ...
            | RunnablePassthrough.assign(
                product_match_status=RunnableLambda(
                    lambda inputs: HANDLERS_MAPPER['check_for_products_matching_customer_preferences'](
                        self.llm,
                        context=inputs["context"],
                        # context=(m.group(0) if (m := re.search(r"Collection:.*?stars;", inputs["context"], re.DOTALL)) else ""),
                        # **self._extract_safe_preferences(inputs),
                        **{k: v for k, v in self._extract_safe_preferences(inputs).items() if k != 'purchase_type'}
                    )
                )
            )
            | RunnableBranch(
                (
                    # lambda x: x['product_match_status'] == 'FOUND',
                    # lambda x: x['product_match_status'].startswith('FOUND'), 
                    lambda x: (print("product_match_status:", x.get('product_match_status')) or True) and x['product_match_status'] == 'FOUND',
