"""
Review Management Serializer for DRF React Gems E-commerce Platform

This module defines a specialized serializer for review management functionality
that includes approval status and is only used by reviewers. This keeps the
main ReviewSerializer clean and focused on general review display.
"""
...
class ReviewManagementSerializer(ReviewSerializer):
    ...
    approved: serializers.BooleanField = serializers.BooleanField(read_only=True)

    class Meta(ReviewSerializer.Meta):
        # Inherit all fields from parent and add approved
        fields = ReviewSerializer.Meta.fields + ['approved']
        read_only_fields = ReviewSerializer.Meta.read_only_fields + ['approved']
